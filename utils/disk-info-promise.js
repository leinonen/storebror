...
function fixTotals(result) {
	var factor = 1024.0;
	if (result.size.unit === 'G' && result.size.value > factor) {
		result.size.unit = 'T';
		result.size.value /= factor;
	}
	if (result.used.unit === 'G' && result.used.value > factor) {
		result.used.unit = 'T';
		result.used.value /= factor;
	}
	if (result.avail.unit === 'G' && result.avail.value > factor) {
		result.avail.unit = 'T';
		result.avail.value /= factor;
	}
	return result;
}
